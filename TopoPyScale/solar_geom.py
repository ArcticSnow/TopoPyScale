...
def get_solar_geom(df_position, start_date, end_date, tstep, sr_epsg="4326", num_threads=None, fname='ds_solar.nc', project_directory='./'):
    ...
    ds = xr.Dataset(
        ...
        coords={
            "point_id": (("point_id"), df_position.index),     # https://github.com/pydata/xarray/issues/2931 xarray.core.merge.MergeError: coordinate point_id shares a name with a dataset dimension, but is not a 1D variable along that dimension. This is disallowed by the xarray data model.
